 ...
 */
public class BCX509CertificateGenerator extends GenericX509CertificateGenerator {
	 ...
	 * 	This *must* be a real java certificate, not a BCX509CertificateObject, as the
	 *  latter has significant problems verifying itself, and we
	 *  can't expect callers to know that and compensate.
			   ...
			   InvalidAlgorithmParameterException 
	{
		...
		try {
			...
			X509CertificateObject certificateObject = 
				new X509CertificateObject(
						new X509CertificateStructure(
								new DERSequence(seqVector)));

			// Convert this certificate to a real 
			// java.security.cert.X509Certificate. If we don't
			// do that, verification fails, which is unexpected
			// by our customers.
			try {
				cert = convert(certificateObject, SUN_PROVIDER);
			} catch (CertificateException e) {
				System.out.println("This should not happen: cannot encode BC certificate and reconstitute it as standard certificate.");
				e.printStackTrace();
			} catch (NoSuchProviderException e) {
				System.out.println("This should not happen: standard provider: " + SUN_PROVIDER + " does not exist.");
				e.printStackTrace();
			}
	...
	// Convert a passed-in certificate object into another certificate
	// generated by a different provider
	public static X509Certificate convert(X509Certificate cert,
										 String provider)
					throws CertificateException, NoSuchProviderException {
		CertificateFactory cf;
		if (provider != null) {
			// Will make one of provider's certificate if it has one.
	...
	/**
	 * We have a lot of problems with the various certificate
	 * classes not wanting to output the actual order of 
	 * the DNs in the certificate. Route all requests
	 * for DNs through these helper functions, which 
	 * on current tests give out the "true" (in-certificate)
	 * order. There may be values in the database, though,
	 * that were put in in incorrect order; make sure not
	 * to try to compare these to those.
	 * 
	 * This function works on both BC certificates and
	 * Java certificates, and doesn't turn things into
	 * OIDs that shouldn't be.
	 */
	public static String getVerbatimSubjectDN(X509Certificate cert) {
		try {
			return encodedDNToString(cert.getSubjectX500Principal().getEncoded());
		} catch (IOException e) {
			System.out.println("This should not happen: cannot decode name encoded from existing certificate! " + e.getMessage());
			e.printStackTrace();
			return null;
		}
	}
	
	public static String getVerbatimIssuerDN(X509Certificate cert) {
		try {
			return encodedDNToString(cert.getIssuerX500Principal().getEncoded());
		} catch (IOException e) {
			System.out.println("This should not happen: cannot decode name encoded from existing certificate! " + e.getMessage());
			e.printStackTrace();
			return null;
	...
	public static X509Name getSubjectX509Name(X509Certificate cert) {
		try {
			return encodedDNToX509Name(cert.getSubjectX500Principal().getEncoded());
		} catch (IOException e) {
			System.out.println("This should not happen: cannot decode name encoded from existing certificate! " + e.getMessage());
			e.printStackTrace();
			return null;
	...
	public static X509Name getIssuerX509Name(X509Certificate cert) {
		try {
			return encodedDNToX509Name(cert.getIssuerX500Principal().getEncoded());
		} catch (IOException e) {
			System.out.println("This should not happen: cannot decode name encoded from existing certificate! " + e.getMessage());
			e.printStackTrace();
			return null;
	...
	public static X509Name encodedDNToX509Name(byte [] encodedDN) throws IOException {
		ASN1InputStream as = new ASN1InputStream(encodedDN);
		DERObject obj = as.readObject();
		X509Name x509Name = new X509Name((ASN1Sequence)obj);
		return x509Name;
	}

	public static String encodedDNToString(byte [] encodedDN) throws IOException {
		X509Name x509Name = encodedDNToX509Name(encodedDN);
