...
public class ContentObject extends GenericXMLEncodable implements XMLEncodable, Comparable<ContentObject> {
	...
	/*
	 * We don't specify a required publisher, and right now we don't enforc
	 * that publisherID is the digest of the key used to sign (which could actuall
	 * be handy to preserve privacy); we just use the key locator and publisherI
	 * combined to look up keys in caches (though for right now we only put keys i
	 * caches by straight digest; would have to offer option to put keys in cache
	 * using some privacy-preserving function as well.
	 *
	 * TODO evaluate when the gap between checking verifier and checkin
	 * publisherID matters. Probably does; could have bogus publisherID, an
	 * then real key locator and content that uses key locator would then verif
	 ...
	 * @see org.ccnx.ccn.impl.encoding.XMLEncodabl
	 *
