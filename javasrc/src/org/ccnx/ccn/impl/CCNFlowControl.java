...
public class CCNFlowControl implements CCNFilterListener {
	...
	public CCNFlowControl(ContentName name, CCNHandle library) throws IOException {
		this(library);
		if (name != null) {
			Log.logger().finest("adding namespace: " + name);
	...
	public CCNFlowControl(CCNHandle library) throws IOException {
		if (null == library) {
			...
			try {
				...
				Log.logger().info("Got ConfigurationException attempting to create a library. Rethrowing it as an IOException. Message: " + e.getMessage());
	...
	public void addNameSpace(ContentName name) throws IOException {
		...
		while (it.hasNext()) {
			...
			if (filteredName.isPrefixOf(name)) {
				Log.logger().info("addNameSpace: not adding name: " + name + " already monitoring prefix: " + filteredName);
		...
		Log.logger().info("addNameSpace: adding namespace: " + name);
		_filteredNames.add(name);
		_library.registerFilter(name, this);
	}
	...
	private void removeNameSpace(ContentName name, boolean all) {
		...
		while (it.hasNext()) {
			...
			if (all || filteredName.equals(name)) {
				_library.unregisterFilter(filteredName, this);
				it.remove();
				Log.logger().finest("removing namespace: " + name);
				break;
			}
		}
	}
	...
	private ContentObject waitForMatch(ContentObject co) throws IOException {
		if (_flowControlEnabled) {
			synchronized (_holdingArea) {
				...
				Log.logger().finest("Holding " + co.name());
				_holdingArea.put(co.name(), co);
				match = _unmatchedInterests.removeMatch(co);
				if (match != null) {
					Log.logger().finest("Found pending matching interest for " + co.name() + ", putting to network.");
					_library.put(co);
					afterPutAction(co);
				}
				if (_holdingArea.size() >= _highwater) {
					...
					do {
						interrupted = false;
						try {
							Log.logger().finest("Waiting for drain");
	...
	public int handleInterests(ArrayList<Interest> interests) {
		synchronized (_holdingArea) {
			for (Interest interest : interests) {
				Log.logger().fine("Flow controller: got interest: " + interest);
				ContentObject co = getBestMatch(interest);
				if (co != null) {
					Log.logger().finest("Found content " + co.name() + " matching interest: " + interest);
					try {
						_library.put(co);
						afterPutAction(co);
					} catch (IOException e) {
						Log.logger().warning("IOException in handleInterests: " + e.getClass().getName() + ": " + e.getMessage());
						Log.warningStackTrace(e);
					}
					
				} else {
					Log.logger().finest("No content matching pending interest: " + interest + ", holding.");
	...
	private ContentObject getBestMatch(Interest interest, Set<ContentName> set) {
		...
		Log.logger().finest("Looking for best match to " + interest + " among " + set.size() + " options.");
			...
			while (_holdingArea.size() > 0) {
				...
				if (_holdingArea.size() == startSize) {
					for(ContentName co : _holdingArea.keySet()) {
						Log.logger().warning("FlowController: still holding: " + co.toString());
					}
	...
	public void clearUnmatchedInterests() {
		Log.logger().info("Clearing " + _unmatchedInterests.size() + " unmatched interests.");
