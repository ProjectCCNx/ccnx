...
public class BasicPolicy implements Policy {
	...
	public boolean update(InputStream stream, boolean fromNet) throws XMLStreamException, IOException {
		...
		Log.logger().info("Policy file update requested");
			...
			case VERSION:
				...
				if (idAttr != null) {
					if (!idAttr.getValue().trim().equals(_repoVersion)) {
						Log.logger().warning("Bad version in policy file: " + idAttr.getValue().trim());
			...
			if (event.isStartElement()) {
				...
				if (started) {
					...
					case NAMESPACE:
						...
						Log.logger().fine("New namespace requested: " + charValue);
						...
						try {
							...
							if (fromNet) {
									if (!ContentName.fromNative(fixSlash(localName)).equals(_localName)) {
										Log.logger().warning("Repository local name doesn't match: request = " + localName);
					...
					case GLOBALNAME:
						...
						if (fromNet) {
							if (!ContentName.fromNative(fixSlash(globalName)).equals(_globalPrefix)) {
								Log.logger().warning("Repository local name doesn't match: request = " + globalName);
	...
	public ContentObject getPolicyContent() {
		try {
			...
			Log.logger().severe("SHOULD NOT HAPPEN: Unexpected MalformedContentNameStringException: " + e.getMessage());
			return null;
		}	
	}
