 ...
 *   form.
 */
public final class WireDecoder extends GenericXMLDecoder {
...
//		long value = peekStartElementAsLong();
	 ...
	 * @throws IOException
	 */
	public final void decode(final InputStream istream) throws IOException {
		initialize();
...
//	private final Element [] _elements = new Element[ELEM_MAX];
	...
	private static int _systemBlockSize = SegmentationProfile.DEFAULT_BLOCKSIZE;

	// BLOB and UDATA now go in their own buffers, so don't really need the full BLOCKSIZE

	private final int _blockSize = _systemBlockSize / 2;
...
//	/**
//	 * This is the Document Object Model.  It is a linear list
//	 * of elements as they appear in the input stream.
//	 *
//	 * @type is the CCN type (DTAG, BLOB, UDATA, CLOSE)
//	 * @value is the appropriate value.  We only support int lengths,
//	 *        even though its a long.  The underlying buffer is only
//	 *        works on ints.
//	 * @position is the position in the underlying byte buffer
//	 *        of the NEXT ELEMENT.  For BLOB or
//	 *        UDATA, it points to the start of the blob/data.  For
//	 *        DTAG or CLOSE, it points to the next tag.
//	 * @author marc
//	 *
//	 */
...
//		public final int position;  // position of NEXT THING
//		public final byte [] blob; 	// blob data
////
////		Element(int type, long value, int position) {
////			this.type = type;
////			this.value = value;
...
////		}
//
//		Element(final int type, final long value, final int position, final byte [] blob) {
//			this.type = type;
//			this.value = value;
//			this.position = position;
//			this.blob = blob;
...
//		@Override
//		public String toString() {
//			return String.format("Type 0x%02X Value 0x%04X Position %08X", type, value, position);
//		}
//	}
...
//			byte b = (byte) next;
			pos = _bytesRead++;
//			System.out.println(String.format("Read byte %02X", next));
