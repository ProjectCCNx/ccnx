...
public class CCNNetworkManager implements Runnable {
	...
	private class PeriodicWriter extends TimerTask {
		public void run() {
			...
			try {
				synchronized (_myInterests) {
					for (Entry<InterestRegistration> entry : _myInterests.values()) {
						...
						if (ourTime > reg.nextRefresh) {
							Log.logger().finer("Refresh interest: " + reg.interest);
				...
				Log.logger().severe("Processing thread failure (Malformed datagram): " + xmlex.getMessage()); 
	...
	private void heartbeat() {
		try {
			...
			Log.logger().warning("Error sending heartbeat packet: " + io.getMessage());
			try {
				if (_channel.isConnected())
					_channel.disconnect();
			} catch (IOException e) {}
	...
	protected abstract class ListenerRegistration implements Runnable {
		...
		public void run() {
			...
			try {
				...
				Log.logger().warning("failed delivery: " + ex.toString());
			} finally {
				synchronized(this) {
		...
		public void deliver() {
			try {
				if (null != this.listener) {
					...
					if (null != results) {
						Log.logger().finer("Interest callback (" + results.size() + " data) for: " + this.interest.name());
						...
						if (null != updatedInterest) {
							Log.logger().finer("Interest callback: updated interest to express: " + updatedInterest.name());
						...
						Log.logger().finer("Interest callback skipped (no data) for: " + this.interest.name());
					}
				} else {
					synchronized (this) {
						if (null != this.sema) {
							Log.logger().finer("Data consumes pending get: " + this.interest.name());
							...
							Log.logger().finest("releasing " + this.sema);
							this.sema.release();
						} 
					}
					if (null == this.sema) {
						// this is no longer valid registration
						Log.logger().finer("Interest callback skipped (not valid) for: " + this.interest.name());
					}
				}
			} catch (Exception ex) {
				Log.logger().warning("failed to deliver data: " + ex.toString());
	...
	protected class Filter extends ListenerRegistration {
		...
		public void deliver() {
			try {
				...
				if (null != results) {								
					// Call into client code without holding any library locks
					Log.logger().finer("Filter callback (" + results.size() + " interests) for: " + name);
					listener.handleInterests(results);
				} else {
					Log.logger().finer("Filter callback skipped (no interests) for: " + name);
				}
			} catch (RuntimeException ex) {
				Log.logger().warning("failed to deliver interest: " + ex.toString());
			}
		}
		@Override
	...
	public CCNNetworkManager() throws IOException {
		...
		if (null != portval) {
			...
			Log.logger().warning("Non-standard CCN agent port " + _port + " per property " + PROP_AGENT_PORT);
		}
		String hostval = System.getProperty(PROP_AGENT_HOST);
		if (null != hostval && hostval.length() > 0) {
			_host = hostval;
			Log.logger().warning("Non-standard CCN agent host " + _host + " per property " + PROP_AGENT_HOST);
		}
		Log.logger().info("Contacting CCN agent at " + _host + ":" + _port);
	...
	public void setTap(String pathname) throws IOException {
		...
		if (pathname != null && pathname.length() > 0) {
			...
			Log.logger().info("Tap writing to " + pathname);
		}
	}
	...
	public ContentObject get(Interest interest, long timeout) throws IOException, InterruptedException {
		Log.logger().fine("get: " + interest);
		InterestRegistration reg = new InterestRegistration(this, interest, null, null);
		expressInterest(reg);
		Log.logger().finest("blocking for " + interest.name() + " on " + reg.sema);
		...
		Log.logger().finest("unblocked for " + interest.name() + " on " + reg.sema);
	...
	public void expressInterest(
		...
		Log.logger().fine("expressInterest: " + interest.name());
	...
	public void cancelInterest(Object caller, Interest interest, CCNInterestListener callbackListener) {
		...
		Log.logger().fine("cancelInterest: " + interest.name());
	...
	public void cancelInterestFilter(Object caller, ContentName filter, CCNFilterListener callbackListener) {
		Log.logger().fine("cancelInterestFilter: " + filter);
		synchronized (_myFilters) {
	...
	protected void write(ContentObject data) throws XMLStreamException {
		...
		writeInner(packet);
		Log.logger().finest("Wrote content object: " + data.name());
	}
	...
	private void writeInner(WirePacket packet) throws XMLStreamException {
		try {
			...
			synchronized (_channel) {
				...
				Log.logger().finest("Wrote datagram (" + datagram.position() + " bytes, result " + result + ")");
				if (null != _tapStreamOut) {
					try {
						_tapStreamOut.write(bytes);
					} catch (IOException io) {
						Log.logger().warning("Unable to write packet to tap stream for debugging");
					}
				}
			}
		} catch (IOException io) {
			// We do not see errors on send typically even if 
			// agent is gone, so log each but do not track
			Log.logger().warning("Error sending packet: " + io.toString());
		}
	}
	...
	InterestRegistration registerInterest(InterestRegistration reg) {
		...
		setupTimers();
		Log.logger().finest("registerInterest for " + reg.interest.name() + " and obj is " + _myInterests.hashCode());
		synchronized (_myInterests) {
	...
	public void run() {
		if (! _run) {
			Log.logger().warning("CCNSimpleNetworkManager run() called after shutdown");
			return;
		}
		...
		Log.logger().info("CCNSimpleNetworkManager processing thread started");
		while (_run) {
			try {
				...
				try {
					if (_selector.select(SOCKET_TIMEOUT) != 0) {
						...
						Log.logger().finest("Read datagram (" + datagram.position() + " bytes)");
						_selector.selectedKeys().clear();
						if (null != _error) {
							Log.logger().info("Receive error cleared");
							_error = null;
						}
					...
					if (null == _error) {
						Log.logger().info("Unable to receive from agent: is it still running?");
					}
					_error = io;
					packet.clear();
				}
				...
				for (ContentObject co : packet.data()) {
					Log.logger().fine("Data from net: " + co.name());
				...
				for (Interest interest : packet.interests()) {
					Log.logger().fine("Interest from net: " + interest);
				...
				Log.logger().severe("Processing thread failure (UNKNOWN): " + ex.getMessage());
                                Log.warningStackTrace(ex);
			}
		}
		...
		synchronized (_myFilters) {
			for (Filter filter : _myFilters.getValues(ireg.interest.name())) {
				if (filter.owner != ireg.owner) {
					Log.logger().finer("Schedule delivery for interest: " + ireg.interest);
