...
public class KeyDerivationFunction {
	...
	public static final byte [] DeriveKeyForNode(
		...
		if (ancestorNodeName.equals(nodeName)) {
			Log.logger().info("We're at the correct node already, will return the original node key.");
		}
	...
	public static final byte [] DeriveKey(byte [] masterKeyBytes, int outputLengthInBits, 
		...
		if (allzeros) {
			Log.logger().warning("Warning: DeriveKey called with all 0's key of length " + masterKeyBytes.length);
		}
		Mac hmac;
		try {
			hmac = Mac.getInstance("HmacSHA256");
		} catch (NoSuchAlgorithmException e1) {
			Log.logger().severe("No HMAC-SHA256 available! Serious configuration issue!");
		...
		if (n < 1) {
			Log.logger().warning("Unexpected: 0 block key derivation: want " + outputLengthInBits + 
		...
		for (int i=1; i <= n; ++i) {
			try {
				...
				try {
					...
					Log.logger().severe("No UTF-8 encoding available! Serious configuration issue!");
				...
				Log.logger().severe("Unexpected IllegalStateException in DeriveKey: hmac should have been initialized!");
				...
				Log.logger().severe("Unexpected ShortBufferException in DeriveKey: buffer should be sufficient!");
