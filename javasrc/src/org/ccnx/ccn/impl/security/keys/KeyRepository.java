...
public class KeyRepository implements CCNFilterListener, CCNInterestListener {
	...
	protected void recordKeyToFile(PublisherPublicKeyDigest id, ContentObject keyObject) {
		...
		if (!keyDir.exists()) {
			if (!keyDir.mkdirs()) {
				Log.logger().warning("recordKeyToFile: Cannot create user CCN key repository directory: " + keyDir.getAbsolutePath());
				return;
			}
		}
		...
		if (keyFile.exists()) {
			Log.logger().info("Already stored key " + id.toString() + " to file.");
		...
		try {
			...
			Log.logger().info("recordKeyToFile: cannot record key: " + id.toString() + " to file " + keyFile.getAbsolutePath() + " error: " + e.getClass().getName() + ": " + e.getMessage());
			return;
		}
		Log.logger().info("Logged key " + id.toString() + " to file: " + keyFile.getAbsolutePath());
	}
		...
		if (null != name) {
			...
			if (null != keyObject) {
				try {
					...
					Log.logger().warning("Unexpected exception " + e.getClass().getName() + ": " + e.getMessage() + ", should not have to decode public key, should have it in cache.");
	...
	public PublicKey getPublicKey(PublisherPublicKeyDigest desiredKeyID, KeyLocator locator, long timeout) throws IOException {
		...
		if (locator.type() != KeyLocator.KeyLocatorType.NAME) {
			Log.logger().info("This is silly: asking the repository to retrieve for me a key I already have...");
			...
			try {
				Log.logger().info("Trying network retrieval of key: " + keyInterest.name());
				keyObject = _networkManager.get(keyInterest, timeout);
			} catch (IOException e) {
				Log.logger().warning("IOException attempting to retrieve key: " + keyInterest.name() + ": " + e.getMessage());
				Log.warningStackTrace(e);
			} catch (InterruptedException e) {
				Log.logger().warning("Interrupted attempting to retrieve key: " + keyInterest.name() + ": " + e.getMessage());
			}
			if (null != keyObject) {
				if (keyObject.signedInfo().getType().equals(ContentType.KEY)) {
					try {
						Log.logger().info("Retrieved public key using name: " + locator.name().name());
					...
					Log.logger().warning("Retrieved an object when looking for key " + locator.name().name() + " at " + keyObject.name() + ", but type is " + keyObject.signedInfo().getTypeName());
				}
			}
		}
		return null;
		...
		while (it.hasNext()) {
			...
			if (null != keyObject) {
				try {
					...
					Log.logger().info("KeyRepository::handleInterests, exception in put: " + e.getClass().getName() + " message: " + e.getMessage());
