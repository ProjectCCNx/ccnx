...
public class EnumeratedNameList implements BasicNameEnumeratorListener {
	...
	public SortedSet<ContentName> getNewData() {
		...
		synchronized(_childLock) {
			while (null == _newChildren) {
				...
				Log.logger().info("Waiting for new data on prefix: " + _namePrefix + " got " + ((null == _newChildren) ? 0 : _newChildren.size())
						+ ".");
			}
	...
	public void waitForData() {
		...
		synchronized(_childLock) {
			while ((null == _children) || (_children.size() == 0)) {
				...
				Log.logger().info("Waiting for data on prefix: " + _namePrefix + " got " + ((null == _newChildren) ? 0 : _newChildren.size())
						+ ".");
			}
		}
	}
	...
	public int handleNameEnumerator(ContentName prefix,
		...
		Log.logger().info(names.size() + " new name enumeration results: our prefix: " + _namePrefix + " returned prefix: " + prefix);
		if (!prefix.equals(_namePrefix)) {
			Log.logger().warning("Returned data doesn't match requested prefix!");
		}
		Log.logger().info("Handling Name Iteration " + prefix +" ");
		...
		synchronized (_childLock) {
			...
			if (!thisRoundNew.isEmpty()) {
				...
				Log.logger().info("New children found: " + thisRoundNew.size() + " total children " + _children.size());
	...
	public static EnumeratedNameList exists(ContentName childName, ContentName prefixKnownToExist, CCNHandle library) throws IOException {
		if ((null == prefixKnownToExist) || (null == childName) || (!prefixKnownToExist.isPrefixOf(childName))) {
			Log.logger().info("Child " + childName + " must be prefixed by name " + prefixKnownToExist);
