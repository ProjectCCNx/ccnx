...
public class CCNNameEnumerator implements CCNFilterListener, CCNInterestListener {
	...
	public void registerPrefix(ContentName prefix) throws IOException {
		synchronized (_currentRequests) {
			...
			ContentName prefixMarked = new ContentName(prefix, CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION);
	...
	public Interest handleContent(ContentObject c, Interest interest) {
		...
		if (interest.name().contains(CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION)) {
			...
			if (c != null) {
				...
				if (responseName==null ) {
					...
					if(getIdFromName(interest.name()) != null && getIdFromName(interest.name()).count() > 0) {
						...
						ContentName prefixWithMarker = new ContentName(prefix, CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION);
				...
				try {
					...
					callback.handleNameEnumerator(interest.name().cut(CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION), names);
	...
	public boolean handleInterest(Interest interest) {
		...
		if (!name.contains(CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION)) {
			//Skip...  we don't handle these
		} else {
			name = name.cut(CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION);
			responseName = new ContentName(name, CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION);
	...
	private ContentName getIdFromName(ContentName name) {
		...
		try {
			int index = name.containsWhere(CommandMarker.COMMAND_MARKER_BASIC_ENUMERATION);
