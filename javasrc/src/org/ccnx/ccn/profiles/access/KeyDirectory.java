...
public class KeyDirectory extends EnumeratedNameList {
	...
	private void initialize() throws IOException {
		if (!VersioningProfile.hasTerminalVersion(_namePrefix)) {
			...
			if (null == latestVersionName) {
				Log.logger().info("Unexpected: can't get a latest version for key directory name : " + _namePrefix);
				getNewData();
				latestVersionName = getLatestVersionChildName();
				if (null == latestVersionName) {
					Log.logger().info("Unexpected: really can't get a latest version for key directory name : " + _namePrefix);
			...
			synchronized (_childLock) {
				...
				if (latestVersionName.count() > 1) {
					Log.logger().warning("Unexpected: NE protocol gave back more than one component!");
				}
		...
		for (ContentName childName : newChildren) {
			...
			if (AccessControlProfile.isWrappedKeyNameComponent(wkChildName)) {
				...
				try {
					...
					Log.logger().info("Unexpected " + e.getClass().getName() + " parsing key id " + DataUtils.printHexBytes(wkChildName) + ": " + e.getMessage());
					// ignore and go on
				}
	...
	public WrappedKeyObject getWrappedKeyForPrincipal(String principalName) throws IOException, XMLStreamException, ConfigurationException {
		...
		if (null == pi) {
			Log.logger().info("No block available for principal: " + principalName);
			return null;
		}
		...
		Log.logger().info("Retrieving wrapped key for principal " + principalName + " at " + principalLink.getTargetName());
	...
	public Link getPreviousKey() throws XMLStreamException, IOException {
		...
		if (!previousKey.available()) {
			Log.logger().info("Unexpected: no previous key link at " + getPreviousKeyBlockName());
			return null;
		}
		...
		if (null == unwrappedKey) {
			...
			if (hasSupersededBlock()) {
				...
				if (null != supersededKeyBlock) {
					...
					Log.logger().info("Attempting to retrieve key " + getName() + " by retrieving superseding key " + supersededKeyBlock.wrappedKey().wrappingKeyName());
					...
					if (null != unwrappedSupersedingKey) {
						...
						Log.logger().info("Unable to retrieve superseding key " + supersededKeyBlock.wrappedKey().wrappingKeyName());
					}
				}

			} else {
				// This is the current key. Enumerate principals and see if we can get a key to unwrap.
				Log.logger().info("At latest version of key " + getName() + ", attempting to unwrap.");
					...
					for (String principal : getPrincipals().keySet()) {
						...
						if (_manager.groupManager().amCurrentGroupMember(principal)) {
							try {
								...
								unwrappedKey = unwrapKeyForPrincipal(principal, principalKey);
								if (null == unwrappedKey) {
									Log.logger().warning("Unexpected: we are a member of group " + principal + " but get a null key.");
									continue;
								}
							} catch (AccessDeniedException aex) {
								Log.logger().warning("Unexpected: we are a member of group " + principal + " but get an access denied exception when we try to get its key: " + aex.getMessage());
								continue;
							}
						}
					}
		...
		if (null != unwrappedKey) {
			...
			if (null != expectedKeyID) {
				...
				if (!Arrays.areEqual(expectedKeyID, retrievedKeyID)) {
					Log.logger().warning("Retrieved and decrypted wrapped key, but it was the wrong key. We wanted " + 
	...
	protected Key unwrapKeyForPrincipal(String principal, Key unwrappingKey) 
		...
		if (null == unwrappingKey) {
			Log.logger().info("Null unwrapping key. Cannot unwrap.");
			return null;
		}
		...
		if (null != wko.wrappedKey()) {
			...
			Log.logger().info("Unexpected: retrieved version " + getPrincipals().get(principal) + " of " + principal + " group key, but cannot retrieve wrapped key object.");
		}
		return unwrappedKey;
	}
	...
	public PrivateKey getPrivateKey() throws IOException, XMLStreamException, 
		...
		if (!hasPrivateKeyBlock()) {
			Log.logger().info("No private key block exists with name " + getPrivateKeyBlockName());
			return null;
		}
		WrappedKeyObject wko = getPrivateKeyObject();
		if ((null == wko) || (null == wko.wrappedKey())) {
			Log.logger().info("Cannot retrieve wrapped private key for " + getPrivateKeyBlockName());
			return null;
		}
		// This should throw AccessDeniedException...
		Key wrappingKey = getUnwrappedKey(wko.wrappedKey().wrappingKeyIdentifier());
		if (null == wrappingKey) {
			Log.logger().info("Cannot get key to unwrap private key " + getPrivateKeyBlockName());
		...
		if (!(unwrappedPrivateKey instanceof PrivateKey)) {
			Log.logger().info("Unwrapped private key is not an instance of PrivateKey! Its an " + unwrappedPrivateKey.getClass().getName());
		} else {
			Log.logger().info("Unwrapped private key is a private key, in fact it's a " + unwrappedPrivateKey.getClass().getName());
		}
	...
	public void addPreviousKeyLink(ContentName previousKey, PublisherID previousKeyPublisher) throws XMLStreamException, IOException, ConfigurationException {
		...
		if (hasPreviousKeyBlock()) {
			Log.logger().warning("Unexpected, already have previous key block : " + getPreviousKeyBlockName());
		}
