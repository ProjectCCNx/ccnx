...
public class Group {
	...
	public KeyDirectory privateKeyDirectory(AccessControlManager manager) throws IOException {
		...
		Log.logger().info("Public key not ready for group: " + friendlyName());
		return null;
	}
	...
	public Timestamp membershipListVersion() throws XMLStreamException, IOException, ConfigurationException {
		...
		if (VersioningProfile.hasTerminalVersion(name)) {
			try {
				...
				Log.logger().warning("Should not happen: VersionMissingException on name where isVersioned is true: " + name + ": " + e.getMessage());
			}
		}
		return null;
	}
	...
	public Key createGroupPublicKey(GroupManager manager, MembershipList ml) 
		...
		try {
			...
			if (manager.getGroupKeyAlgorithm().equals(AccessControlManager.DEFAULT_GROUP_KEY_ALGORITHM)) {
				Log.logger().severe("Cannot find default group public key algorithm: " + AccessControlManager.DEFAULT_GROUP_KEY_ALGORITHM + ": " + e.getMessage());
		...
		try {
			...
			Log.logger().warning("Unexpected -- InvalidKeyException wrapping key with keys we just generated! " + e.getMessage());
			throw e;
		}
		...
		for (Link lr : ml.membershipList().contents()) {
			try {
				...
				if (!latestPublicKey.available()) {
					Log.logger().warning("Could not retrieve public key for " + lr.targetName());
					continue;
				}
				...
				Log.logger().warning("Could not retrieve public key for principal " + lr.targetName() + ", skipping.");
			} catch (VersionMissingException e) {
				Log.logger().warning("Unexpected: public key name not versioned! " + latestPublicKey.getCurrentVersionName() + ", unable to retrieve principal's public key. Skipping.");
			}
		}
	...
	public String toString() {
		...
		if ((null == _groupMembers) || (!_groupMembers.available())) {
			...
			try {
				...
				Log.logger().warning("Unexpected " + e.getClass().getName() + " exception in getMembershipListName(): " + e.getMessage());
