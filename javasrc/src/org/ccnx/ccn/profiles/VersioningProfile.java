...
public class VersioningProfile implements CCNProfile {
	...
	public static boolean isLaterVersionOf(ContentName laterVersion, ContentName earlierVersion) throws VersionMissingException {
		...
		Log.logger().warning("SEMANTICS CHANGED: if experiencing unexpected behavior, check to see if you want to call isLaterVerisionOf or startsWithLaterVersionOf");
	...
	public static ContentObject getLatestVersion(ContentName startingVersion, 
		...
		while (true) {
			...
			if (co == null) {
				Log.logger().info("Null returned from getLatest for name: " + startingVersion);
				return null;
			}
			...
			if (startsWithLaterVersionOf(co.name(), startingVersion)) {
				...
				Log.logger().info("Got latest version: " + co.name());
				// Now need to verify the block we got
				if (verifier.verify(co)) {
					return co;
				}
				Log.logger().warning("VERIFICATION FAILURE: " + co.name() + ", need to find better way to decide what to do next.");
			} else {
				Log.logger().info("Rejected potential candidate version: " + co.name() + " not a later version of " + startingVersion);
			}
	...
	public static ContentObject getFirstBlockOfLatestVersion(ContentName startingVersion, 
		...
		Log.logger().info("getFirstBlockOfLatestVersion: getting version later than " + startingVersion);
		...
		if (null != result){
			Log.logger().info("getFirstBlockOfLatestVersion: retrieved latest version object " + result.name() + " type: " + result.signedInfo().getTypeName());
			...
			if (CCNVersionedInputStream.isFirstSegment(startingVersion, result, startingSegmentNumber)) {
				Log.logger().info("getFirstBlockOfLatestVersion: got first block on first try: " + result.name());
				return result;
			}
			...
			Log.logger().info("getFirstBlockOfLatestVersion: Have version information, now querying first segment of " + startingVersion);
			...
			Log.logger().info("getFirstBlockOfLatestVersion: no block available for later version of " + startingVersion);
		}
		return result;
	}
