...
static int shortDelayMicros = 100;      // short delay for quick reschedule
...
// noteRemoteHash remembers a remote hash (given by the hash cache entry),
...
static void
kickHeartBeat(struct SyncRootStruct *root) {
    if (root != NULL)
            ...
            switch (data->kind) {
                ...
                case SRI_Kind_NodeFetch: {
                    ...
                    if (debug >= CCNL_INFO && showHighLevel)
                        showCacheEntry1(root, "Sync.$NodeFetch, reply received", highWhy, ce);
    ...
    if (name != NULL) {
        ...
        if (res < 0) {
            ...
            if (debug >= CCNL_INFO) {
                ...
                free(hex);
                if (showHighLevel)
                    showCacheEntry1(root, "Sync.$NodeFetch", "request", ce);
            }
        }
...
CloseUpdateCoding(struct SyncUpdateData *ud) {
    ...
    if (deltas != NULL) {
        ...
        struct SyncHashCacheEntry *ceStop = NULL;
        if (hashL != NULL)
            ceStop = SyncHashLookup(root->ch, hashL->buf, hashL->length);
        ud->ceStop = ceStop;
            ...
            if (cres < 0 || hp == NULL || hs == 0) {
                ...
                if (!isCovered(ce)) {
                    ...
                    if (ce != NULL && ce->ncR != NULL) {
                        nc = ce->ncR;
                        highWhy = "existing but not covered";
                        if (debug >= CCNL_INFO)
                ...
                if (debug >= CCNL_INFO && showHighLevel) {
                    ...
                    snprintf(temp, sizeof(temp),
                             ...
                             highWhy);
                    showCacheEntry2(root, "Sync.$RootAdvise", temp,
                                    SyncHashLookup(root->ch, hash->buf, hash->length),
                                    ce);
                }
            }
            if (failed) {
...
SyncSendRootAdviseInterest(struct SyncRootStruct *root) {
    ...
    if (res >= 0) {
        ...
        if (debug >= CCNL_INFO) {
            ...
            if (showHighLevel)
                showCacheEntry1(root, "Sync.$RootAdvise", "interest sent", ce);
        }
        return 1;
    } else {
...
SyncUpdateRoot(struct SyncRootStruct *root) {
    ...
    if (hash != NULL)
        ud->ceStart = SyncHashLookup(root->ch, hash->buf, hash->length);
