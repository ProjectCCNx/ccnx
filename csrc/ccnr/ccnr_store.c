 ...
 */
static int
r_store_keyfetcher(struct ccn_schedule *sched,
    void *clienth,
    struct ccn_scheduled_event *ev,
    int flags)
{
    ...
    (void)(sched);
    if ((flags & CCN_SCHEDULE_CANCEL) != 0 || h->keyfetch == NULL || h->keyfetch->n == 0) {
        h->keyfetcher = NULL;
        return(0);
    }
    if (1) {
        content = r_store_content_from_accession(h, h->keyfetch->buf[0]);
        if (content == NULL)
            ccn_indexbuf_remove_first_match(h->keyfetch, h->keyfetch->buf[0]);
        else {
            ccnr_debug_ccnb(h, __LINE__, "keyfetch", NULL,
                        content->key, content->size);
            ccn_indexbuf_remove_first_match(h->keyfetch, h->keyfetch->buf[0]);
        }
    }
...
/**
 * Start up the key fetcher if it is not currently active.
 */
PUBLIC void
r_store_keyfetch_needed(struct ccnr_handle *h, int init_delay_usec)
{
    if (h->keyfetcher == NULL)
        h->keyfetcher = ccn_schedule_event(h->sched, init_delay_usec,
                                           &r_store_keyfetcher, NULL, 0);
}
