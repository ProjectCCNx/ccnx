...
TOPSUBDIRS = csrc `cat local.subdirs 2>/dev/null || :`
...
default all: _always
	...
	mkdir -p ./lib ./bin
#	test -d ./include || (mkdir -p include/ccnr && mkdir -p include/sync && cp -r csrc/ccnr/*.h include/ccnr && cp -r csrc/sync include/sync)
...
distfile: tar
	...
	mkdir ccnr-$(VERSION)
	( cd ccnr-$(VERSION) && tar xf ../ccnr.tar && $(MAKE) fixupversions VERSION=$(VERSION) && $(MAKE) MD5 SHA1 )
	# Build the documentation
	( cd ccnr-$(VERSION) && $(MAKE) dist-docs 2>&1) > ccnr-$(VERSION)-documentation.log
	tar cf ccnr-$(VERSION).tar ccnr-$(VERSION)
	gzip -9 ccnr-$(VERSION).tar
	ls -l ccnr-$(VERSION).tar.gz
