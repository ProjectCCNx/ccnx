...
public class ContainerGUItest extends JFrame implements
	...
	public ContainerGUItest() {
		...
		try {
			node = new DefaultMutableTreeNode(new IconData(ICON_FOLDER, null,
					new Name("/files", ContentName
							.fromNative("/parc.com/files"))));
			
			node.add(new DefaultMutableTreeNode(new IconData(ICON_FOLDER, null,
					new Name("/junk", ContentName
							.fromNative("/parc.com/junk")))));
		 ...
		 */
			...
			public void actionPerformed(ActionEvent evt) {
				...
				System.out.println("Writing a file to the repo "
				...
				try {
					System.out.println("Sending file " + selectedPrefix + "/"
							+ file.getName());
					RepositoryOutputStream ros = repoguy.repoOpen(ContentName
					...
					readFile(ContentName.fromNative(selectedPrefix + "/"
							+ file.getName()));
					getne.registerNameForResponses(ContentName
							.fromNative(selectedPrefix + "/" + file.getName()));
	...
	public void sendFile(String filename) {
		...
		try {
			...
			RepositoryOutputStream ros = repoguy.repoOpen(ContentName
					.fromNative(filename), null, repoguy.getDefaultPublisher());
	...
	public void readFile(ContentName name) {
		...
		try {
			ContentObject testContent = getLibrary.get(new Interest(name),
					10000);
